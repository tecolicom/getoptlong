...
*   **`aliases`:** (Optional) One or more option aliases, each following a `-` (e.g., `v`).  Multiple aliases can be specified, separated by `|` (e.g., `long|s|t`).  If only a short name is defined (e.g., `[s:]`), it also serves as the base for the variable name, unless a `destination_var` is provided (see Section 3.3).  Prefixing an alias with `~` creates a **negative alias** — a short option that negates the option (equivalent to `--no-<name>`).  For example, `[sweep|w|~W]` makes `-W` behave like `--no-sweep`.
...
*   **Variable Behavior:**
    ...
    *   Specifying with a negative alias (e.g., `-W` for `[sweep|w|~W]`): Variable is reset to empty string, equivalent to `--no-sweep`
