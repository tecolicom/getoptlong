...
# test_helper.bash sources getoptlong.sh, which is needed by prefix.sh
load 'test_helper'
...
@test "prefix.sh: shows help with --help" {
...
@test "prefix.sh: 2 times: echo hello" {
...
@test "prefix.sh: --count=1: echo single" {
    run "$SCRIPT_UNDER_TEST" --count=1 echo single
    assert_success
    assert_output "single"
}

@test "prefix.sh: paragraph (-p): echo line (default newline)" {
...
@test "prefix.sh: --paragraph=---: echo segment" {
...
@test "prefix.sh: message BEGIN (-m BEGIN=Start)" {
...
@test "prefix.sh: message END (--message END=Finish)" {
...
@test "prefix.sh: message EACH (-m EACH=Cycle)" {
...
@test "prefix.sh: multiple messages (BEGIN, EACH, END)" {
...
@test "prefix.sh: command with spaces: echo hello world" {
    run "$SCRIPT_UNDER_TEST" -c 1 echo "hello world"
    assert_success
    assert_output "hello world"
}

@test "prefix.sh: command with its own options: ls -a /dev/null (check for null)" {
...
@test "prefix.sh: no arguments (should show help/error and fail)" {
    run "$SCRIPT_UNDER_TEST"
    assert_success
    assert_output ''
}

@test "prefix.sh: -i 0.01 (sleep, hard to test duration, just runs)" {
...
@test "prefix.sh: multiple sleep intervals -i 0.01 -i 0.02 (runs 3 times)" {
...
@test "prefix.sh: -x (set-x) option (check for trace output)" {
...
@test "prefix.sh: -d (debug level 1)" {
