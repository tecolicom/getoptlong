...
**Note:** A help option (defaulting to `--help`, `-h`) is automatically available.
See Section "4. Help Message Generation and Customization" for details on its
...
description in the auto-generated help message.
...
*   **INITIAL VALUE:** Specified as the value of the associative array element (e.g., `]=0`, `]=/dev/stdout`).
    This becomes the default value if the option is not provided on the command line.
    Behavior varies by type if not specified (e.g., flags default to empty string or `0`, arrays to empty array).

After parsing with `getoptlong parse` and setting variables with `eval "$(getoptlong set)"`,
shell variables corresponding to the options (either default names or custom `destination_var` names) are populated.
The `PREFIX` configuration can add a common prefix to these variable names.
Hyphens (`-`) in `long_name` are converted to underscores (`_`) for default variable names (e.g., `--very-verbose` becomes `$very_verbose` or `$PREFIX_very_verbose`).
...
*   **Customization:**
    You can change the name, alias, and description of this automatic help option using:
    ...
    *   Explicitly defining an option with the same name (e.g., `[help|h #... ]=`) in `OPTS`
        will also override the default, effectively acting like `&HELP`.

*   **Disabling Automatic Help:**
    To completely disable the automatic help option, you can set `HELP` to an empty string
    and ensure no `&HELP` key or conflicting option definition exists:
    ```bash
...
The synopsis, or usage line (e.g., "Usage: myscript [options] <file>"),
displayed at the beginning of the help message can be customized.
...
*   **Methods for Customization:**
    ...
    2.  **`USAGE` parameter in `getoptlong init` (Lower Precedence):**
        ...
        ```
    *   If neither `&USAGE` (in `OPTS`) nor `USAGE` (in `init`) is specified, a synopsis line
        is not displayed by default when help is triggered automatically. However, see
        `getoptlong help` below.
...
You can manually display the help message from anywhere in your script using the
`getoptlong help` command. This is useful for showing help on specific errors
or via a custom help flag not handled by the automatic mechanism.
...
*   **Behavior:**
    ...
    *   **`[SYNOPSIS_ARGUMENT]`:** (Optional) If provided, this string is used as the
        synopsis line for this specific invocation of the help message. It takes precedence
        over any `&USAGE` or `USAGE` settings.
        ```bash
    ...
    *   The automatic help option (e.g., `--help`) internally calls `getoptlong help` without
        a `SYNOPSIS_ARGUMENT`, so it relies on `&USAGE` or `USAGE` for the synopsis.
...
option's value is internally set.
...
*   **Combining with Other Types:**
    ...
    The `-` specifier must appear *after* any primary type specifier (`+`, `:`, `?`, `@`, `%`).
    It can be combined with the callback modifier `!`. The primary type specifier cannot be omitted when using `-`,
    as this would make it ambiguous with a hyphen in a long option name.
...
*   **Help Message:**
    ...
    Options defined with the `-` passthrough specifier will be described
...
for other parts of the process or for passing to sub-commands. It's crucial
to include a primary type specifier (`+`, `:`, `?`, `@`, `%`) before the `-` modifier.
...
*   **`[CONFIGURATIONS...]`**: Optional configuration parameters
    ...
    *   **`DESTINATION=<array_name>` (Deprecated)**:

        This setting is deprecated. Use per-option destination variables as described in
        Section "3.1. Basic Syntax" and "5.2. Specifying Destination Variable" instead.
        Previously, this stored parsed option values in the specified associative array
        `<array_name>`. Keys were the long names of options. For example,
        with `DESTINATION=OptValues` and option `--my-opt val`, `OptValues[my-opt]="val"`
        would have been stored. This array had to be declared beforehand with `declare -A <array_name>`.

    *   **`HELP=<SPEC>`**:

        Customizes the automatically added help option. See Section "4.1. Automatic Help Option"
        for details. Default: `help|h#show help`.

    *   **`USAGE=<string>`**:

        Specifies the synopsis string for the help message. See Section "4.3.1. Customizing Synopsis (`USAGE` Setting)"
        for details. If not set, the synopsis is typically not displayed unless `getoptlong help` is called with an argument.
...
including how the optional `SYNOPSIS` argument is handled.
...
*   **`&HELP=<SPEC>`**
    ...
    *   Description: Customizes the definition of the auto-generated help option.
        Overrides the `HELP` parameter of `getoptlong init`.
        See Section "4.1. Automatic Help Option" for more details.
    *   Example: `[&HELP]="show-usage|u#Usage guide"`
    *   Default: `help|h#show help`

*   **`&USAGE=<string>`**

    *   Description: Specifies the usage (Synopsis) string to be displayed at the
        beginning of the help message. Overrides the `USAGE` parameter of `getoptlong init`.
        See Section "4.3.1. Customizing Synopsis (`USAGE` Setting)" for more details.
