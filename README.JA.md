...
- [2. 基本的な使い方](#2-基本的な使い方)
  ...
  - [2.2. 標準的なマルチステップでの使い方](#22-標準的なマルチステップでの使い方)
    - [2.2.1. オプション定義配列の作成](#221-オプション定義配列の作成)
    - [2.2.2. ライブラリの読み込み](#222-ライブラリの読み込み)
...
`getoptlong.sh` は、シェルスクリプト内でコマンドラインオプションを解析するために設計されたBashライブラリです。組み込みの `getopts` コマンドに代わる堅牢で柔軟な代替手段を提供し、GNUスタイルのロングオプション、オプションと引数の柔軟な配置、様々な引数タイプ（必須、オプション、配列、ハッシュ）、データバリデーション、コールバックメカニズム、そしてヘルプメッセージの自動生成をサポートします。その目標は、Bashにおいて複雑になりがちなコマンドライン引数処理を簡素化し、スクリプトをよりユーザーフレンドリーで保守しやすくすることです。

## 2. 基本的な使い方

`getoptlong.sh` は、スクリプトにオプション解析を統合するための主要な2つの方法を提供します：シンプルなワンライナースタイルと、標準的なマルチステップスタイルです。

### 2.1. シンプルなワンライナーでの使い方

多くの一般的なユースケースでは、`getoptlong.sh` はライブラリを読み込んで、オプションを定義し、引数を解析し、変数を設定する一行で呼び出すことができます。これは最も手軽に始める方法です。

**手順:**

1.  **オプション配列を定義する:** オプション定義を格納するBashの連想配列（例：`OPTS`）を作成します。

2.  **読み込んで実行する:** `.` (source) コマンドに続けて `getoptlong.sh`、オプション配列の名前、そして `"$@"` を指定します。
...
次に、スクリプト内から `source` コマンド（または `.` コマンド）を使って `getoptlong.sh` ファイルを読み込みます。これにより、`getoptlong` 関数とそのサブコマンドが利用可能になります。
...
*   **変数割り当ての詳細:**
    *   フラグオプション（例：`[verbose|v]`）は、初めて使用されると変数を `1` に設定し、その後の使用でインクリメントします。
...
*   **`名前`:** (必須) `--` に続くロングオプション名（例：`verbose`）。ハイフンを含むことができます（例：`very-verbose`）。これは、オプションの値が格納される変数名のベースにもなります（ハイフンはアンダースコアに置換されます）。ただし、`格納先変数`が指定されている場合を除きます（セクション3.3参照）。

*   **`エイリアス`:** (任意) `-` に続く1つ以上のオプションエイリアス（例：`v`）。複数のエイリアスは `|` で区切って指定できます（例：`long|s|t`）。短い名前のみが定義された場合（例：`[s:]`）、`格納先変数`が指定されていない限り、それが変数名になります（セクション3.3参照）。
...
`getoptlong parse` で解析し、`eval "$(getoptlong set)"` で変数を設定した後、オプションに対応するシェル変数（デフォルト名またはカスタムの`格納先`名）が設定されます。`PREFIX` 設定は、これらの変数名に共通のプレフィックスを追加できます。`名前`に含まれるハイフン (`-`) は、デフォルトの変数名ではアンダースコア (`_`) に変換されます（例：`--very-verbose` は `$very_verbose` または `${PREFIX}_very_verbose` になります）。
...
*   **振る舞い:** 引数が浮動小数点数でない場合（例：`123.45` はOK、`1.2e-3` のような指数表記は受け付けません）、エラーメッセージが表示され、スクリプトは終了します。
...
*   **カスタマイズ:** この自動ヘルプオプションの名前、エイリアス、説明は以下を使用して変更できます:
    ...
    1.  **`OPTS` 配列内の `&HELP` キー (最高優先度):**
        ...
        ```bash
        declare -A OPTS=(
            [&HELP]="show-usage|u#使用方法を表示して終了します。"
            # ... 他のオプション定義 ...
        )
        getoptlong init OPTS
        ```

        これで、`--show-usage` または `-u` でヘルプメッセージが表示されます。
    ...
    2.  **`getoptlong init` の `HELP` パラメータ (低優先度):**
        ...
        `OPTS` に `&HELP` が設定されていない場合、`--manual` または `-m` でヘルプが表示されます。
...
より説明的なロングオプション名（例：`--backup-location`）を使用すると、自動生成されるメッセージの可読性が向上します。
...
*   **カスタマイズ方法:**
    ...
    *   `&USAGE`（`OPTS`内）も `USAGE`（`init`内）も指定されていない場合、ヘルプが自動的に表示される場合は、デフォルトでは使い方（Synopsis）の行は表示されません。ただし、下記の `getoptlong help` を参照してください。
...
コールバック関数を使用すると、特定のオプションの解析時に関数を実行できます。これにより、単純な値の設定を超えた複雑な処理をオプション解析に統合できます。コールバック関数は、オプション定義に `!` 接尾辞を追加するか、`getoptlong callback` コマンドを使用して登録されます。
...
*   **ユースケース:**
    ...
    *   特定のオプションの指定時に、他のデフォルト値を動的に変更する。
...
このコマンドは、自動ヘルプオプション（例：`--help`）の呼び出し時にも内部的に実行されます。
